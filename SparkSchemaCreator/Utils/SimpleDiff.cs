...
namespace SparkSchemaCreator.Utils
{
    public class SimpleDiff<T>
    {
        ...
        private void CalculatePostSkip()
        {
            ...
            while (_postSkip < leftLen && _postSkip < rightLen &&
                   _postSkip < (leftLen - _preSkip) && _compareFunc != null &&
        ...
        private void CalculatePreSkip()
        {
            int leftLen = _left.Count;
            int rightLen = _right.Count;
            while (_preSkip < leftLen && _preSkip < rightLen && _compareFunc != null &&
        ...
        {
            if (leftIndex > 0 && rightIndex > 0 && _compareFunc != null &&
            ...
            else
            {
                if (rightIndex > 0 &&
                    (leftIndex == 0 ||
                     (_matrix != null && _matrix[leftIndex, rightIndex - 1] >= _matrix[leftIndex - 1, rightIndex])))
                {
        ...
        private void CreateLCSMatrix()
        {
            ...
            for (int i = 1; i <= _left.Count - totalSkip; i++)
            {
                ...
                {
                    _matrix[i, j] = (_compareFunc != null && _compareFunc(_left[leftIndex], _right[rightIndex - 1]))
                                        ? _matrix[i - 1, j - 1] + 1
    ...
    public class DiffEventArgs<T>(DiffType diffType, T lineValue, int leftIndex, int rightIndex) : EventArgs
    {
        public DiffType DiffType { get; set; } = diffType;

        public T LineValue { get; private set; } = lineValue;
        public int LeftIndex { get; private set; } = leftIndex;
        public int RightIndex { get; private set; } = rightIndex;
    }
    ...
    public class Lines : List<Line>, IEquatable<Lines>
    {
        ...
        public static Lines Load(IList<string> strLines)
        {
            Lines lines = [];
        ...
        public Lines Expand(int from, int to)
        {
            ...
            for (int i = from; i <= to; i++)
            {
                if (this[i].state != DiffType.Deleted)
                    result.Add(this[i]);
                if (this[i].subLines != null && this[i].subLines?.Expand() is Lines lines)
    ...
    /// </summary>
    public class ConflictedLine(Lines version1, Lines version2) : Line("?")
    {
        public readonly Lines version1 = version1;
        public readonly Lines version2 = version2;
    }
}
