...
namespace SparkSchemaCreator.Controls
{
    ...
    internal partial class NativeToolStripRenderer(ToolbarTheme theme) : ToolStripSystemRenderer
    {
        ...
        internal static partial class NativeMethods
        {
            ...
            public static partial int GetThemeMargins(IntPtr hTheme, IntPtr hdc, int iPartId, int iStateId, int iPropId, IntPtr rect, out MARGINS pMargins);
        }
        ...
        Padding GetThemeMargins(Graphics dc, MarginTypes marginType)
        {
            try
            {
                IntPtr hDC = dc.GetHdc();
                if (renderer != null && 0 == NativeMethods.GetThemeMargins(renderer.Handle, hDC, renderer.Part, renderer.State, (int)marginType, IntPtr.Zero, out NativeMethods.MARGINS margins))
        ...
        {
            if (EnsureRenderer() && renderer != null)
            {
        ...
        {
            int partId = (item != null && item.IsOnDropDown) ? (int)MenuParts.PopupItem : (int)MenuParts.BarItem;

            if (renderer != null)
            {
                renderer.SetParameters(MenuClass, partId, GetItemState(item));

                if (Theme == ToolbarTheme.CommunicationsToolbar && item != null)
                    return item.IsOnDropDown ? renderer.GetColor(ColorProperty.TextColor) : Color.FromArgb(220, 220, 220);
        ...
        protected override void OnRenderButtonBackground(ToolStripItemRenderEventArgs e)
        {
            ...
            if(Theme == ToolbarTheme.MediaToolbar && e.Item.Selected)
            {

                int transparency = e.Item.Pressed ? 29 : 100;

                Rectangle rect = new(Point.Empty, new Size(e.Item.Width, e.Item.Height - 1));
                using (SolidBrush b = new(Color.FromArgb(transparency, Color.White)))
                    e.Graphics.FillRectangle(b, rect);

                Rectangle rect2 = new(Point.Empty, new Size(e.Item.Width - 1, e.Item.Height - 2));
                using Pen p = new(Color.FromArgb(50, Color.White));
        ...
        {
            if (e.ToolStrip != null && e.ToolStrip.IsDropDown && EnsureRenderer() && renderer != null)
            {
                renderer.SetParameters(MenuClass, (int)MenuParts.PopupSeparator, 0);
                Rectangle rect = new(e.ToolStrip.DisplayRectangle.Left, 0, e.ToolStrip.DisplayRectangle.Width, e.Item.Height);
        ...
        protected override void OnRenderItemCheck(ToolStripItemImageRenderEventArgs e)
        {
            if (EnsureRenderer() && renderer != null)
            {
                ...
                if (e.Item.RightToLeft == RightToLeft.Yes && e.ToolStrip != null)
