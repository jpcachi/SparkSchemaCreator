    ...
    public class FileTextSource : TextSource, IDisposable
    {
        ...
        private void UnloadUnusedLines()
        {
            ...
            for (int i = 0; i < Count; i++)
                if (base.lines[i] is Line line && !line.IsChanged && Math.Abs(i - iFinishVisibleLine) > margin)
                {
                    base.lines[i] = null;
                    count++;
                }
        ...
        public override void SaveToFile(string fileName, Encoding enc)
        {
            ...
            var fileNameWithoutExtension = Path.GetFileNameWithoutExtension(fileName) + ".tmp";
            var tempFileName = dir == null ? fileNameWithoutExtension : Path.Combine(dir, fileNameWithoutExtension);
            ...
            using (StreamWriter sw = new StreamWriter(tempFs, enc))
            {
                ...
                for (int i = 0; i < Count; i++)
                {
                    ...
                    bool lineIsChanged = false;
                    if (lines[i] is Line _line && _line.IsChanged)
                    {
                        lineIsChanged = true;
                        line = _line.Text;
                    }
        ...
        public override int GetLineLength(int i) => lines[i]?.Count ?? 0;
