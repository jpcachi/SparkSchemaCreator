...
namespace SparkSchemaCreator.Controls.FastColoredTextBox
{
    ...
    public class AutocompleteListView : UserControl, IDisposable
    {
        ...
        AutocompleteMenu Menu => Parent as AutocompleteMenu ?? new AutocompleteMenu(tb);
        int oldItemCount = 0;
        ...
        void Tb_KeyPressed(object? sender, KeyPressEventArgs e)
        {
            ...
            if ((Menu?.Visible ?? false) && !backspaceORdel)
        ...
        static void ResetTimer(System.Windows.Forms.Timer timer)
        {
            timer.Stop();
            timer.Start();
        }
        ...
        void Tb_KeyDown(object? sender, KeyEventArgs e)
        {
            ...
            if (!Menu.Visible)
            {
                if (sender is FastColoredTextBox tb && tb.HotkeysMapping.TryGetValue(e.KeyData, out FCTBAction value) && value == FCTBAction.AutocompleteMenu)
                {
                    DoAutocomplete();
                    e.Handled = true;
                }
        ...
        private void SetToolTip(AutocompleteItem autocompleteItem)
        {
            ...
            if (Parent != null)
            {
                ...
                if (Parent != null && (PointToScreen(Location).X + MaxToolTipSize.Width + 105) < Screen.FromControl(Parent).WorkingArea.Right)
    ...
    {
        public AutocompleteItem? Item { get; internal set; }
        public required FastColoredTextBox Tb { get; set; }
    }
}
