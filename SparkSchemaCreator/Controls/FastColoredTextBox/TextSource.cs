...
namespace SparkSchemaCreator.Controls.FastColoredTextBox
{
    ...
    public class TextSource: IList<Line?>, IDisposable
    {
        ...
        IEnumerator IEnumerable.GetEnumerator() => lines as IEnumerator ?? lines.GetEnumerator();
            ...
            if (count > 0)
                if (IsNeedBuildRemovedLineIds)
                    for (int i = 0; i < count; i++)
                        if (this[index + i] is Line line)
                            removedLineIds.Add(line.UniqueId);
            //
